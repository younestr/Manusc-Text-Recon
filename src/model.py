...
class Model:
    ...
    def setup_cnn(self) -> None:
        ...
        for i in range(num_layers):
            ...
            conv_norm = batch_norm_layer(conv, training=True)
    ...
    def setup_rnn(self) -> None:
        """Create RNN layers using Keras."""
        # Squeeze the CNN output to remove unnecessary dimensions
        rnn_in3d = tf.squeeze(self.cnn_out_4d, axis=[2])

        # Define parameters for the LSTM layers
        num_hidden = 256
        num_layers = 2  # Number of LSTM layers

        # Create a sequential stack of LSTM layers
        for i in range(num_layers - 1):  # Add all but the last layer (unidirectional)
            rnn_in3d = tf.keras.layers.LSTM(num_hidden, return_sequences=True)(rnn_in3d)

        # Add the last LSTM layer as a bidirectional LSTM
        bidirectional_output = tf.keras.layers.Bidirectional(
            tf.keras.layers.LSTM(num_hidden, return_sequences=True)
        )(rnn_in3d)

        # Concatenate the forward and backward outputs
        concat = tf.expand_dims(bidirectional_output, axis=2)

        # Project the output to the number of characters (including blank)
        kernel = tf.keras.layers.Conv2D(
            filters=len(self.char_list) + 1,
            kernel_size=(1, 1),
            padding='same'
        )(concat)

        # Squeeze the output to remove the extra dimension
        self.rnn_out_3d = tf.squeeze(kernel, axis=[2])
